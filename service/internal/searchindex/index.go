...
func New() *Index {
	return &Index{docs: map[string]Doc{}, titlePos: map[string]map[string][]int{}, snippetPos: map[string]map[string][]int{}, bodyPos: map[string]map[string][]int{}, urlPos: map[string]map[string][]int{}, headingsPos: map[string]map[string][]int{}, total: 0}
}
...
func stemEn(tok string) string {
	...
	if strings.HasSuffix(s, "ing") && n > 5 {
		base := s[:n-3]
		if lb := len(base); lb >= 2 && base[lb-1] == base[lb-2] {
			base = base[:lb-1]
		}
		return base
	}
...
func (ix *Index) Search(q string, limit int) []Result {
	...
	if len(req) > 0 {
		...
		for tok := range req {
			...
			for vt := range variants {
				for id := range ix.titlePos[vt] {
					docsSet[id] = struct{}{}
				}
				for id := range ix.snippetPos[vt] {
					docsSet[id] = struct{}{}
				}
			}
			if first {
	...
	for tok := range qTokens {
		...
		for vt, mult := range variants {
			...
			for id := range ix.bodyPos[vt] {
				docsSet[id] = struct{}{}
			}
			for id := range ix.urlPos[vt] {
				docsSet[id] = struct{}{}
			}
			for id := range ix.headingsPos[vt] {
				docsSet[id] = struct{}{}
			}
	...
	for id := range cand {
		...
		for _, ph := range phrases {
			...
			if hasPhrase(ix.titlePos, id, toks) || hasPhrase(ix.snippetPos, id, toks) || hasPhrase(ix.bodyPos, id, toks) || hasPhrase(ix.urlPos, id, toks) || hasPhrase(ix.headingsPos, id, toks) {
				...
				if strings.Contains(corpus, strings.ToLower(ph)) {
					scores[id] += 0.5
				}
			}
		}
...
func (ix *Index) FilteredSearch(q string, source string, lang string, from string, to string, sortBy string, page int, size int) ([]Result, int, map[string]int64) {
	...
	for _, r := range results {
		...
		if lang != "" && strings.ToLower(strings.TrimSpace(d.Lang)) != strings.ToLower(strings.TrimSpace(lang)) {
			continue
		}
		if hasFrom || hasTo {
			if dt, err := parseDate(d.Date); err == nil {
				if hasFrom && dt.Before(fromT) {
					continue
				}
				if hasTo && dt.After(toT) {
					continue
				}
			}
		}
...
func (ix *Index) FilteredSearchTransport(q string, meta map[string]string, sortBy string, page int, size int) ([]Result, int, map[string]int64) {
	...
	for _, r := range results {
		...
		if wantFrom != "" && strings.ToLower(d.Meta["from"]) != wantFrom {
			continue
		}
		if wantTo != "" && strings.ToLower(d.Meta["to"]) != wantTo {
			continue
		}
		if wantDepart != "" && !strings.Contains(d.Meta["depart"], wantDepart) {
			continue
		}
		if wantReturn != "" && !strings.Contains(d.Meta["return"], wantReturn) {
			continue
		}
		filtered = append(filtered, r)
		facets[d.Source] = facets[d.Source] + 1
...
func NewStore() *Store {
	return &Store{Web: New(), Images: New(), Videos: New(), News: New(), Transport: New()}
}
...
func (ix *Index) vocabulary() map[string]struct{} {
	...
	for t := range ix.titlePos {
		vocab[t] = struct{}{}
	}
	for t := range ix.snippetPos {
		vocab[t] = struct{}{}
	}
	for t := range ix.bodyPos {
		vocab[t] = struct{}{}
	}
	for t := range ix.urlPos {
		vocab[t] = struct{}{}
	}
	for t := range ix.headingsPos {
		vocab[t] = struct{}{}
	}
	return vocab
}
