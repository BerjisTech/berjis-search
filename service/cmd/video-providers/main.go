...
func fetchYouTubeSearch(apiKey, q, channelID string) []map[string]string {
    ...
    resp, err := http.Get(u)
    if err != nil { log.Printf("youtube search error: %v", err); return nil }
    defer resp.Body.Close()
    if resp.StatusCode/100 != 2 { b,_ := io.ReadAll(resp.Body); log.Printf("youtube search %d: %s", resp.StatusCode, string(b)); return nil }
    var out YTSearchResponse
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { log.Printf("youtube decode: %v", err); return nil }
    ...
    for _, it := range out.Items {
        ...
        date := now
        if !it.Snippet.PublishedAt.IsZero() { date = it.Snippet.PublishedAt.UTC().Format(time.RFC3339) }
...
func fetchDailymotionSearch(q string) []map[string]string {
    if strings.TrimSpace(q) == "" { return nil }
    v := url.Values{}
    v.Set("search", q)
    v.Set("limit", "25")
    ...
    resp, err := http.Get(u)
    if err != nil { log.Printf("dailymotion search error: %v", err); return nil }
    defer resp.Body.Close()
    if resp.StatusCode/100 != 2 { b,_ := io.ReadAll(resp.Body); log.Printf("dailymotion search %d: %s", resp.StatusCode, string(b)); return nil }
    var out DMSearchResponse
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { log.Printf("dailymotion decode: %v", err); return nil }
    res := []map[string]string{}
